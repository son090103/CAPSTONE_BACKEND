...
module.exports.getNotifications = async (req, res) => {
    try {
        ...
        const result = await notificationService.getNotifications(leaderId);
...
module.exports.markAsRead = async (req, res) => {
    try {
        ...
        const leaderId = res.locals.user.id;
        const result = await notificationService.markAsRead(id, leaderId);
...
module.exports.markAllAsRead = async (req, res) => {
    try {
        const leaderId = res.locals.user.id;
        const result = await notificationService.markAllAsRead(leaderId);
        return res.status(200).json({
            success: true,
...
module.exports.getUnreadCount = async (req, res) => {
    try {
        const leaderId = res.locals.user.id;
        const count = await notificationService.getUnreadCount(leaderId);
